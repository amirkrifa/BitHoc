...
public:
	...
	static time_t get_time(time_t *)
	{
		SYSTEMTIME time;
		::GetLocalTime(&time);
		time_t temp;
		temp=time.wSecond+time.wMinute*60+time.wHour*60*60;
		return temp;
	}
	
	static tm* get_localtime(time_t *)
	{
		SYSTEMTIME time;
		::GetLocalTime(&time);
		tm * temp_tm= (tm*)malloc(sizeof(tm));
		temp_tm->tm_hour=time.wHour;
		temp_tm->tm_min=time.wMinute;
		temp_tm->tm_sec=time.wSecond;
		temp_tm->tm_mday=time.wDay;
		temp_tm->tm_year=time.wYear;
		temp_tm->tm_mon=time.wMonth;
		return temp_tm;
