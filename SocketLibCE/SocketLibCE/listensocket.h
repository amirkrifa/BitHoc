...
public:
	...
	,m_bHasCreate(false)
	{
		if (use_creator)
		{
			m_creator = new X((SocketHandler&)h);
	...
	// Added by Amir Krifa
	X* GetCreator(){return this->m_creator;};
	~ListenSocket() {
		if (m_creator)
		{
			delete m_creator;
	...
	int Bind(SocketAddress& ad,const std::string& protocol,int depth) {
		SOCKET s;
		s = CreateSocket(ad.GetFamily(), SOCK_STREAM, protocol);
		if ( s == INVALID_SOCKET)
		{
			return -1;
		}
	...
	void OnRead()
	{
		...
		Socket *tmp = m_bHasCreate ? m_creator -> Create() : new X((SocketHandler&)Handler());
#ifdef ENABLE_IPV6
		tmp -> SetIpv6( IsIpv6() );
#endif
		tmp -> SetParent(this);
